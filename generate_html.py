    ...
    <style>
        ...
        * {{
        ...
        .header-tag-link:hover {{
            color: var(--accent);
            text-shadow: 0 0 12px rgba(210, 44, 54, 0.5), 0 0 30px rgba(210, 44, 54, 0.2);
        }}
...
<body>
        ...
        <div class="header-content">
            <div class="header-eyebrow">
                ...
                <div class="lang-switcher">
                    ...
                    <button id="ambient-audio-btn" class="ambient-btn" aria-label="Sonido ambiente">Escuchar Sonido Ambiente</button>
                </div>
            </div>
                ...
                <div class="modal-grid">
                    ...
                    <div class="modal-main-content">
                        <div class="modal-section credits-section" id="modal-credits-section">
                            <h4 class="section-title">Ficha técnica</h4>
    ...
    <script>
        ...
        let activeFadeAnim = null;
        function fadeAudioVolume(audio, targetVolume, duration) {{
            if (!audio) return;
            if (activeFadeAnim) cancelAnimationFrame(activeFadeAnim);
            ...
            function step(currentTime) {{
                ...
                audio.volume = Math.max(0, Math.min(1, startVolume + (distance * ease)));
                
                if (progress < 1) {{
                    activeFadeAnim = requestAnimationFrame(step);
                }} else {{
                    activeFadeAnim = null;
                }}
            }}
        ...
        function openModal(idx) {{
            ...
            if (movie.youtube_id) {{
                ...
                iframe.src = `https://www.youtube.com/embed/${{movie.youtube_id}}?autoplay=1&mute=0&controls=0&loop=1&playlist=${{movie.youtube_id}}&rel=0&modestbranding=1&iv_load_policy=3&disablekb=1&fs=0&enablejsapi=1`;
                
                // Asegurar que el volumen baja enviando el comando repetidamente durante 3 segundos
                // Ya que el reproductor de YouTube puede tardar en estar listo para recibir mensajes
                let volumeAttempts = 0;
                if (window.ytVolumeInterval) clearInterval(window.ytVolumeInterval);
                window.ytVolumeInterval = setInterval(() => {{
                    if (iframe.contentWindow) {{
                        iframe.contentWindow.postMessage('{{"event":"command","func":"setVolume","args":[10]}}', '*');
                    }}
                    volumeAttempts++;
                    if (volumeAttempts > 15) clearInterval(window.ytVolumeInterval); // Detener tras 3 segundos
                }}, 200);
            ...
            // Bajar volumen ASMR si está sonando (suavemente)
            if (window.ambientAudio && window.isAmbientPlaying) {{
                fadeAudioVolume(window.ambientAudio, 0.05, 800); // 800ms de transición
            }}
        ...
        function closeModal() {{
            ...
            // Recuperar volumen ASMR si estaba sonando (suavemente)
            if (window.ambientAudio && window.isAmbientPlaying) {{
                fadeAudioVolume(window.ambientAudio, 0.4, 1500); // 1.5s de transición (más agradable)
            }}
        ...
        ambientBtn.addEventListener('click', () => {{
            ...
            if (window.isAmbientPlaying) {{
                ...
                ambientBtn.style.background = 'rgba(210, 44, 54, 0.25)';
                ambientBtn.style.boxShadow = '0 0 18px var(--accent-glow)';
